...
async function approveRefund(row) {
  const noReturn = row.type === 'REFUND_ONLY' || row.sourceOrderStatus !== 'COMPLETED'
  try {
    await ElMessageBox.confirm(noReturn ? '该订单无需退货，同意后将直接退款。' : '用户已确认收货，同意后需要用户寄回商品。', '同意售后', {
      type: 'warning',
  ...
  await merchantApi.approveRefund(row.id, { remark: noReturn ? '商家同意仅退款，系统直接退款' : '商家同意售后，请寄回商品' })
